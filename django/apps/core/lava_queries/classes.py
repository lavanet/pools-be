...
class LavaQuery:
    ...
    @classmethod
    def _try_query(cls, method='abci_query', path='', data='', height=0):
    ...
    @classmethod
    def query(cls, method='abci_query', path='', data='', height=0):
        response = cls._try_query(method=method, path=path, data=data, height=height)
        if response.status_code != 200:
            raise LavaQueryException(
                f'Path[{path}]: Unexpected status code [{response.status_code}, {response.text}]')
        try:
